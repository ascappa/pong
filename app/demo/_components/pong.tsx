...
export function Pong() {
  ...
  useEffect(() => {
    ...
    const opponentScore = add([text("0"), pos(width() - 20, center().y), anchor("topright"), {value: 0}])
    ...
    onUpdate(async () => {
      if(ball.pos.y < height() * 2 / 3 && ball.exists()) {
        opponentBar.moveTo(ball.pos.x - opponentBar.width / 2, opponentBar.pos.y, width() / 2 +50)
      }
